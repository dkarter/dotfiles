...
# @raycast.title PDQ: Open Graphite PR from Preview Env
# @raycast.mode silent
...
# Documentation:
# @raycast.description Open the Graphite Pull Request (PR) from a Preview Env
...
if [[ $url =~ ^https://pr-([0-9]+)\.houston\.pdq\.tools(/.*)?$ ]]; then
  pr="${BASH_REMATCH[1]}"
  open "https://app.graphite.com/github/pr/pdq/houston/$pr"
else
